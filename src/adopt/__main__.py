...
from adopt.resources import get_resource_path
from adopt.exceptions import CodedError, UnknownError


LOGGER = logging.getLogger(name='adopt')
...
def process(input_file: Path, output_file: Path, log_file: Path):
    ...
    """
    LOGGER.info(f'Running adopt {__version__}')
...
def main():
    parser = argparse.ArgumentParser(
        prog=f'adopt {__version__}',
        description='Set of pragmatic tools to automate working with Azure Devops',
    )
    parser.add_argument('input_file', help='Input file to run adopt')
    parser.add_argument('output_file', help='Output of adopt will be written here')
